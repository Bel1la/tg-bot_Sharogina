...
while i < (len(arrCateg)):
    ...
    def subNews(message):
        ...
        print(arrCateg.index(message.text.replace('/news_', '', 1)))
        print(arrCateg[id])
        # j=0
...
def send_news(message):
    ...
    addUser(message.from_user.id)
    bot.send_message(message.from_user.id, "выберите, из какой категории хотите узнать новости: /news_"+' /news_'.join(subInfo(message)))


# информация
@bot.message_handler(commands=['my_subscribes'])
def send_subInfo(message):
    bot.send_message(message.from_user.id, "вы подписаны на: " + ', '.join(subInfo(message)))
...
def sub(message):
    bot.send_message(message.from_user.id, {"для подписки доступны темы: /" + ' /'.join(arrCateg) + "\n любое нажатие на название темы приведет к подписке!!" })
...
def unsub(message):
    bot.send_message(message.from_user.id, {"чтобы отписаться выберите тему: /UN_" + ' /UN_'.join(subInfo(message)) + "\n любое нажатие на название темы приведет к отписке!!"})
